...
const { Button } = require('urlbar-notification-button');
const { TelemetryLog } = require('lib/TelemetryLog.js');

const panelWidth = 400;
const panelHeight = 192;
const buttonSettings = {
  id: 'security-advisor-button',
  className: 'security-advisor-button',
  message: 'Your account could be at risk!',
  styleLocation: 'recommendation/buttonStyle.css',
  iconLocation: 'recommendation/firefox.svg',
};
...
class Advisor {
  ...
  showPanel(button) {
    button.button.classList.add('security-advisor-panelopen');
    this.panel.show({
      position: button,
    });
  }
  ...
  handlePanelHide() {
    ...
    button.button.classList.remove('security-advisor-panelopen');
  }
  ...
  getButton(win) {
    ...
    const button = new Button(viewFor(win), this.showPanel, buttonSettings);
    button.window = win;
