...
class Advisor {
  constructor() {
    ...
    if (Preferences.isSet('services.sync.username')) {
      this.notifyPanelUserHasSync();
    }
    Preferences.observe('services.sync.username', this.notifyPanelUserHasSync.bind(this));
  }
  ...
  notifyPanelUserHasSync() {
    if (Preferences.isSet('services.sync.username')) {
      this.panel.port.emit('userHasSync');
    }
  }
